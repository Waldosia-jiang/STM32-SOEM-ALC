                           ...
                           if (wkc > 0)
                           {
                              ...
                              else
                              {
                                 ...
                                 if ((aSDOp->Command) == ECT_SDO_ABORT) /* SDO abort frame received */
                                    ecx_SDOerror(context, slave, index, subindex, etohl(aSDOp->data.ldata[0]));
                                 else
                ...
                boolean CA, int psize, void *p, int Timeout)
{
   ...
   if ((psize <= 4) && !CA)
   {
      ...
      if (wkc > 0)
      {
         ...
         if (wkc > 0)
         {
		 uint8 mbxtype = (aSDOp->MbxHeader.mbxtype & 0x0f);
		 uint8 CANOpen = (etohs(aSDOp->CANOpen) >> 12);
		 /* response should be CoE, SDO response, correct index and subindex */
            if ((mbxtype == ECT_MBXT_COE) && 
                (CANOpen == ECT_COES_SDORES) &&
                 (aSDOp->Index == SDOp->Index) &&
   ...
   if (wkc > 0)
   {
      ...
      if (wkc > 0) /* succeeded to read slave response ? */
      {
         ...
         else
         {
            if ((aSDOp->Command) == ECT_SDO_ABORT) /* SDO abort frame received */
            {
               ecx_SDOerror(context, slave, 0, 0, etohl(aSDOp->data.ldata[0]));
            }
            else
            {
...
int ecx_readODlist(ecx_contextt *context, uint16 Slave, ec_ODlistt *pODlist)
{
   ...
   SDOp->data.wdata[0] = htoes(0x01); /* all objects */
   ...
   if (wkc > 0)
   {
      ...
      do
      {
         stop = (boolean)TRUE; /* assume this is last iteration */
         ec_clearmbx(&MbxIn);
         /* read slave response */
         ...
         if (wkc > 0)
         {
                ...
                ((aSDOp->Opcode & 0x7f) == ECT_GET_ODLIST_RES))
            {
               ...
               for (i = 0; i < n; i++)
               {
                  pODlist->Index[sp + i] = etohs(aSDOp->data.wdata[i + offset]);
               }
               sp += n;
            ...
            else
            {
               ...
               {
                  ecx_SDOinfoerror(context, Slave, 0, 0, etohl(aSDOp->data.ldata[0]));
                  stop = (boolean)TRUE;
               }
               else
               {
...
int ecx_readODdescription(ecx_contextt *context, uint16 Item, ec_ODlistt *pODlist)
{
   ...
   SDOp->Reserved = 0;
   SDOp->Fragments = 0; /* fragments left */
   SDOp->data.wdata[0] = htoes(pODlist->Index[Item]); /* Data of Index */
   ...
   if (wkc > 0)
   {
      ...
      if (wkc > 0)
      {
             ...
             ((aSDOp->Opcode & 0x7f) == ECT_GET_OD_RES))
         {
            ...
            pODlist->DataType[Item] = etohs(aSDOp->data.wdata[1]);
            pODlist->ObjectCode[Item] = aSDOp->data.bdata[5];
            pODlist->MaxSub[Item] = aSDOp->data.bdata[4];

            strncpy(pODlist->Name[Item] , (char *)&aSDOp->data.bdata[6], n);
         ...
         else
         {
            if (((aSDOp->Opcode & 0x7f) == ECT_SDOINFO_ERROR)) /* SDO info error received */
            {
               ecx_SDOinfoerror(context, Slave,pODlist->Index[Item], 0, etohl(aSDOp->data.ldata[0]));
            }
            else
            {
...
int ecx_readOEsingle(ecx_contextt *context, uint16 Item, uint8 SubI, ec_ODlistt *pODlist, ec_OElistt *pOElist)
{
   ...
   SDOp->Reserved = 0;
   SDOp->Fragments = 0;      /* fragments left */
   SDOp->data.wdata[0] = htoes(Index);      /* Index */
   SDOp->data.bdata[2] = SubI;       /* SubIndex */
   SDOp->data.bdata[3] = 1 + 2 + 4; /* get access rights, object category, PDO */
   ...
   if (wkc > 0)
   {
      ...
      if (wkc > 0)
      {
             ...
             ((aSDOp->Opcode &  0x7f) == ECT_GET_OE_RES))
         {
            ...
            pOElist->ValueInfo[SubI] = aSDOp->data.bdata[3];
            pOElist->DataType[SubI] = etohs(aSDOp->data.wdata[2]);
            pOElist->BitLength[SubI] = etohs(aSDOp->data.wdata[3]);
            pOElist->ObjAccess[SubI] = etohs(aSDOp->data.wdata[4]);

            strncpy(pOElist->Name[SubI] , (char *)&aSDOp->data.wdata[5], n);
         ...
         else
         {
            if (((aSDOp->Opcode & 0x7f) == ECT_SDOINFO_ERROR)) /* SDO info error received */
            {
               ecx_SDOinfoerror(context, Slave, Index, SubI, etohl(aSDOp->data.ldata[0]));
            }
            else
            {
